...
def train(data_dir, model_dir, args):
    ...
    # -- model
    # model = load_model(saved_model_dir, num_classes, device).to(device)
    ...
    # -- loss & metric
    focal_loss = create_criterion("focal")  # default: cross_entropy
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        loss_value = 0
        matches = 0
        # if epoch >= 10 and loss_changed == False:
        #     criterion = create_criterion('f1')  # default: cross_entropy
        #     loss_changed = True
        for idx, train_batch in enumerate(train_loader):
            ...
            loss = f1_loss(outs, labels) + focal_loss(outs, labels)
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                loss_item = f1_loss(outs, labels).item() + focal_loss(outs, labels).item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_decay_step', type=int, default=10, help='learning rate scheduler deacy step (default: 15)')
    ...
    parser.add_argument('--saved_model_dir', type=str, default=os.environ.get('SM_MODEL_DIR', './model'))
